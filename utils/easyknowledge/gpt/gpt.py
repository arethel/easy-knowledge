...
question_template =  messages = [
    # {"role": "system", "content": "Вы являетесь преподавателем, который задает вопросы к параграфу, предоставленному студентом, а в конце говорите ответ. Важно, чтобы вопрос состоял из одного предложения и в начале вопросо стояло <?>, а в начале ответа <!>."},
    {"role": "system", "content": "You are a system, which generate ONE question and ONE answer for this question based on provided paragraph using paragraph's language. IMPORTANT!!! THAT BEFORE QUESTION THERE MUST BE <?> AND BEFORE ANSWER MUST BE <!> (FOR EXAMPLE '<?>Question? <!>Answer.'). QUESTION AND ANSWER MUST BE IN ONE YOUR MESSAGE!!!"},
    ...
    # {"role": "assistant", "content": "<?> Что Альтман обнаружил и где использовал метод, благодоря которому обнаружил это? <!> Альтман обнаружил нейрогенез взрослых, благодоря тому, что использовал методику на взрослом мозге."},
    ]
...
def create_messages(template, content):
    """Creates a messages to the provided template."""
...
def create_question(content):
    ...
    messages = create_messages(question_template, content)
    
    return generate_response(messages)['choices'][0]['message']['content']

def create_summary(content):
    """Creates a short summary of a long piece of text."""
    messages = create_messages(summary_template, content)
